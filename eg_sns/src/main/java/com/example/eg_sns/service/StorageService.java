...
public class StorageService {
	...
	public String store(MultipartFile multipartFile) {
		...
		// 格納先の相対パス
		Path filePath = Paths.get("src/main/resources/static/assets/img/" + fileName);
	...
	public static boolean isImageFile(MultipartFile multipartFile) {
		...
		try {
			File file = new File("src/main/resources/static/assets/img/" + fileName);
			// 拡張子を除いたファイル名を取得
			file.createNewFile();
    ...
    public StorageService(PostImagesRepository postImagesRepository) {
        this.postImagesRepository = postImagesRepository;
    }

    /**
     * posts_idに基づく画像URLを取得
     *
     * @param postsId ポストID
     * @return 画像URLのリスト
     */
    public List<String> getImageUrlsByPostId(Long postsId) {
        List<PostImages> postImages = postImagesRepository.findByPostsId(postsId);
